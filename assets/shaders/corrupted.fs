...
// use this command to test shader ingame
// watch shader Mods/ArchRail/Assets/Shaders/corrupted.fs
...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    number fac = 0.8 + 0.9*sin(11.*uv.x+4.32*uv.y + corrupted.r*12. + cos(corrupted.r*5.3 + uv.y*4.2 - uv.x*4.));
    number fac2 = 0.5 + 0.5*sin(8.*uv.x+2.32*uv.y + corrupted.r*5. - cos(corrupted.r*2.3 + uv.x*8.2));
    number fac3 = 0.5 + 0.5*sin(10.*uv.x+5.32*uv.y + corrupted.r*6.111 + sin(corrupted.r*5.3 + uv.y*3.2));
    number fac4 = 0.5 + 0.5*sin(3.*uv.x+2.32*uv.y + corrupted.r*8.111 + sin(corrupted.r*1.3 + uv.y*11.2));
    number fac5 = sin(0.9*16.*uv.x+5.32*uv.y + corrupted.r*12. + cos(corrupted.r*5.3 + uv.y*4.2 - uv.x*4.));
    ...
    tex.r = tex.r*(1.31 + (0.015*corrupted.y));
    // reduce the green channel and **SLIGHTLY** dependant on the rotation of the card
    tex.g = tex.r*0.12;
    // greatly reduce the blue channel
    tex.b = tex.b*(1.31 + (0.015*corrupted.x));

    tex.b = tex.b/pow(uv.x,uv.y);
    tex.g = (tex.g/1.9)/(1+(0.11*tex.b));
    tex.r = tex.r*(tex.r/(1+(0.05*tex.g*tex.b*uv.x*uv.y)));
    tex.g = 3.5/(tex.b/(sin(uv.x)/cos(uv.y)));
    tex.r = tex.r-uv.y;
    tex.b = tex.b-uv.x;
    tex.g = tex.g-cos(uv.x*uv.y);
