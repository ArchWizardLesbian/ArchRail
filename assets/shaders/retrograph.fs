...
extern PRECISION vec2 retrograph;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    float warp = fract(cos((retrograph.y + time)+(uv.y*985.))*sin((retrograph.y + time)+uv.x));

    // Time varying pixel color
    vec3 col = vec3(warp/(1.+(0.65*uv.y)),clamp(warp+(cos((retrograph.y + time))-sin(uv.y)),0.25,0.75),warp/(1.+(0.75*uv.y)));

    // Output to screen
    colour = vec4(col,clamp(retrograph.x, 1.0, 1.0));

    float brighten = 0.55+(0.01*uv.x);

    colour.b = colour.b + (colour.r*0.1);
    colour.r = colour.r / 10.;
    if (colour.b > (colour.g+colour.r)/1.5) {
        colour.r = colour.r+(colour.b*brighten);
        colour.g = colour.g+(colour.b*brighten);
    }
