...
extern PRECISION vec2 goldsheet;
...
vec4 effect(vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords)
{
    ...
    uv = rotate_uv(uv, 90.0*sin(truetime/360.));
    uv = abs(2.*uv - 1.);
    ...
    vec3 col = max((warp*sin(truetime+uv.x))*cos(truetime+uv.y),warp*0.5);

    col.x = (col.x * col.y) / col.z;
    col.z = col.z + (col.y * 0.5);
    col.y = col.y * 0.25;
    
    col = (fract(col*uv.x)*fract(col*uv.y));
    
    col.z = col.z / 4.;
    col.y = col.z + col.y;
    col.x = col.x + ((col.y*col.z)*5.3);
    col.z = col.r / (4.5);
    col.y = col.r / (2.);

    float alphaset = 1.;

    if (col.x+col.y+col.z < 2.5) {
        alphaset = 0.4;
    }

    // Output to screen
    colour = vec4(col,clamp(goldsheet.x, alphaset, alphaset));
